...
pub enum ParseError {
    ...
    NoUserMessages,
    #[error("Session contains no real assistant messages (only synthetic CLI messages)")]
    NoRealAssistantMessages,
...
/// Synthetic assistant events are injected by the Claude Code CLI itself, with
/// no model response behind them: API errors (`isApiErrorMessage: true`, e.g.
/// spend limit or 429), and any other locally generated assistant text, which
/// carries the `model: "<synthetic>"` sentinel (e.g. "No response requested.").
fn is_synthetic_assistant_event(event: &Value, model: Option<&str>) -> bool {
    event
...
struct ParseState {
    ...
    /// True once a non-synthetic assistant event is seen. Synthetic events
    /// (`isApiErrorMessage`, `model == "<synthetic>"`) are generated locally by
    /// the CLI, so a session with only those never produced a model response.
    /// See [`is_synthetic_assistant_event`].
    has_real_assistant_message: bool,
    /// True once a synthetic assistant event is seen.
    has_synthetic_assistant_message: bool,
...
impl ParseState {
    fn new() -> Self {
        Self {
            ...
            has_real_assistant_message: false,
            has_synthetic_assistant_message: false,
            messages: Vec::new(),
            tool_calls: Vec::new(),
            subagents: Vec::new(),
    ...
    fn handle_assistant_event(
        ...
        if is_synthetic_assistant_event(event, model.as_deref()) {
            self.has_synthetic_assistant_message = true;
        } else {
            self.has_real_assistant_message = true;
        }
    ...
    fn finish(
        ...
        // Exclude sessions where the assistant never ran: every assistant event
        // is synthetic, generated by the CLI itself (e.g. spend limit reached).
        // A session with no assistant event at all (e.g. freshly started) stays
        // indexable.
        if self.has_synthetic_assistant_message && !self.has_real_assistant_message {
            return Err(ParseError::NoRealAssistantMessages.into());
        }
...
mod tests {
    ...
    #[test]
    fn parse_rejects_session_with_only_synthetic_api_error_assistant_messages() {
        let file = create_temp_session(&[
            r#"{"type":"user","timestamp":"2024-01-01T00:00:00Z","sessionId":"s-synth","message":{"content":"Process this job offer."}}"#,
            r#"{"type":"assistant","timestamp":"2024-01-01T00:00:01Z","sessionId":"s-synth","isApiErrorMessage":true,"message":{"model":"<synthetic>","content":[{"type":"text","text":"You've hit your monthly spend limit"}]}}"#,
        ]);
        ...
        assert!(matches!(
            result.unwrap_err().downcast_ref::<ParseError>(),
            Some(ParseError::NoRealAssistantMessages)
        ));
    }

    #[test]
    fn parse_accepts_session_with_synthetic_error_and_real_assistant_message() {
        let file = create_temp_session(&[
            r#"{"type":"user","timestamp":"2024-01-01T00:00:00Z","sessionId":"s-mixed","message":{"content":"Fix the build"}}"#,
            r#"{"type":"assistant","timestamp":"2024-01-01T00:00:01Z","sessionId":"s-mixed","isApiErrorMessage":true,"message":{"model":"<synthetic>","content":[{"type":"text","text":"You've hit your monthly spend limit"}]}}"#,
            r#"{"type":"assistant","timestamp":"2024-01-01T00:00:02Z","sessionId":"s-mixed","message":{"model":"claude-sonnet-4-5","content":[{"type":"text","text":"On it"}]}}"#,
        ]);

        let parser = ClaudeCodeParser;
        let parsed = parser.parse(file.path()).unwrap();

        // The synthetic error message stays visible in the transcript.
        assert!(
            parsed
                .messages
                .iter()
                .any(|m| m.content.contains("spend limit"))
        );
    }

    #[test]
    fn parse_accepts_session_with_user_message_and_no_assistant_event() {
        let file = create_temp_session(&[
            r#"{"type":"user","timestamp":"2024-01-01T00:00:00Z","sessionId":"s-pending","message":{"content":"Just started"}}"#,
        ]);

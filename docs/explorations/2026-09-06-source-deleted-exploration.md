...
| GNOME shell search | Include missing sessions, ranked purely on message-text relevance like any other result. Activation calls `open-session` (it never resumes), so a missing-source session does the whole job of a shell result. No marker in the result line: the description is short and, with excerpts on, is replaced by the match snippet; the session's persistent banner carries the state the moment it opens. |
...
- System search in `crates/core/src/database/shell_search.rs`: include missing sessions (add the `source_missing` column to the two grouped queries; neither needs a new `WHERE`). Do not prefix the rendered description — leave `ShellSearchMetadata::render` and the excerpt branch untouched. Keep `interface.rs`'s existing "Session unavailable" fallback (no DB row) distinct from the in-app "Source missing" state (row retained). Analytics includes missing sessions; no active-only scope unless a specific analysis declares one.

## Implementation acceptance checks

1. For **each supported AI assistant and both OpenCode storage modes**, index a fixture, remove the session's authoritative source, and rescan: retain content and evidence, mark absence, and keep the row in the default list with its marker while the resume and teammate-ambiguity paths exclude it. A repeated scan must preserve the first detection time.
...
6. Check internal search, GNOME shell search, pins, pagination, analytics, and navigation counts. Missing sessions appear in the in-app list, search, analytics, and counts, and the counts match the visible list. Query the shell provider over a private bus with a term only a missing session matches: its id comes back and the description is unprefixed in both `show_excerpts` modes; activating it opens the session with its banner. A missing teammate must not make a present candidate ambiguous.

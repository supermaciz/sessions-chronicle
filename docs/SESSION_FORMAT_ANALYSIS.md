...
- **Codex shared history (confirmed contract, incomplete investigation)**: [Types inspected at `ac192cd`](https://github.com/openai/codex/blob/ac192cd7937b0d73edc6dffe009940ae53782dd4/codex-rs/protocol/src/protocol.rs) define `history_base.{thread_id,end_ordinal_exclusive,end_byte_offset}`. Here `thread_id` identifies the referenced rollout, and the two boundaries delimit its inherited prefix. Related fields are `forked_from_ordinal_exclusive` and `subagent_history_start_ordinal`. Local parsing ignores them, so missing inherited content is a **likely** consequence. Prefix lookup, chained references, missing files, compressed offsets, and revert/child projection remain **unknown**; this is not yet a reconstruction specification. See [Codex history modes and shared prefixes](session-formats/codex.md#history-modes-and-shared-prefixes).
...
5. **Test parser with edge cases**:
   ...
   - Investigate Codex `history_base` reconstruction separately before defining parser behavior
   - Empty sessions
...
**Status (2026-09-06 Codex watch pass)**: Persistence policy verified at 0.153.4; protocol, recorder, response-item types, and compression inspected at `ac192cd7937b0d73edc6dffe009940ae53782dd4`. Documented the paginated-message gap and bounded `history_base` evidence; corrected existing archive/compression and direct-parent support. No personal-session sampling or test execution in this pass; activation prevalence and introduction versions remain unknown.

...
- `build-aux/dev.maciz.sessionschronicle.json:11` — add `--own-name=dev.maciz.sessionschronicle.SearchProvider` to `finish-args`.
- `data/dev.maciz.sessionschronicle.desktop.in.in` — `DBusActivatable=true`.
- `src/main.rs:35` — `--session <id>` and `--search <query>` clap args, plus app-side deep-linking in `app.rs`.

## Risks

- **Deep-linking does not exist.** `ActivateResult` needs "open the app, navigate to session X" — no D-Bus activation, no `--session` arg, no navigation entry point today. This is the largest hidden cost in issue #189 and it is a **prerequisite**, not a configuration concern. If it slips, the provider ships as a result list where clicking a result opens the app on the default view — a feature that looks finished in a screenshot and is dead in the hand. Don't ship that half.
...
- `--sessions-dir tests/fixtures`, index, then from the overview: type 2 characters → **no results, no SQLite query** (assert with a query counter or `strace`-level check that the 3-char floor short-circuits before `open_connection`).

...
1. `collab_close_end`
2. `collab_waiting_end`
3. `collab_agent_interaction_end`

Within the same event family, later events win.

The stored `result_summary` should prefer concise completed output text when the
status payload provides it. If the status is non-terminal or non-textual, keep
the existing summary rather than overwriting it with weaker data.
...
- a partial parent-only case
- a partial child-only case

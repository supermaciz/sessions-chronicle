...
This design implements Proposal C from the exploration: `AdwBanner` for persistent warnings, enriched toasts for transient feedback, sidebar status dots for at-a-glance source health, and an enhanced empty state showing detected source paths.

**Explicitly out of scope:**
- "Details" button on banner or toast (deferred to issue #94)
...
pub enum SourceStatus {
    ...
    Empty,      // path exists but 0 sessions found
...
`IndexingWorkerOutput` loses its `Copy` derive (because `PerSourceResult` contains `PathBuf`).
...
`index_all_incremental` and `index_all_full_reindex` return a `Vec<PerSourceResult>` instead of an aggregated `IndexingStats`. Each per-assistant call builds its `PerSourceResult` by checking `path.exists()` and deriving `SourceStatus` from the counters:

| `path_exists` | `indexed` | `errors` | Status |
|---|---|---|---|
| `false` | — | — | `NotFound` |
| `true` | `0` | `0` | `Empty` |
| `true` | `> 0` | `0` | `Indexed` |
...
## 2. AdwBanner for Persistent Warnings

### Placement

The banner is inserted in the main view's vertical `gtk::Box` (content of the `ToolbarView`), between the `SearchBar` and the `OverlaySplitView`.
...
- **Revealed** when `IndexingCompleted` arrives with at least one source in `Degraded`, `Failed`, or `NotFound`.
- **Hidden** when the next `IndexingCompleted` arrives with no problems.
- **Hidden** on manual dismiss (built-in `AdwBanner` close button).
...
| Situation | Message |
|---|---|
| N errors, 0 sources missing | `"Indexing completed with N errors"` |
| 0 errors, M sources missing | `"M session sources not found"` |
| N errors + M sources missing | `"Indexing completed with N errors — M sources not found"` |

### No action button

No "Details" button in this phase. The button will be wired to the diagnostics dialog in issue #94.
...
|---|---|---|---|
| Success | `errors == 0`, all sources OK or `NotFound` | `"Index rebuilt — N sessions"` (unchanged) | 3s |
...
- **Failure:** toast only (worker produced no `per_source`, so no banner).
...
When no sessions exist and indexing has completed, the `AdwStatusPage` "No Sessions Yet" (in `session_list.rs`) gains a child widget (`set_child`) listing detected sources and their status.

The child is a `gtk::ListBox` with one `adw::ActionRow` per assistant:

- **Title:** assistant display name (e.g. "Claude Code")
- **Subtitle:** resolved path in monospace (truncated with ellipsis), or "Directory not found" if absent
...
The existing description `"Your AI coding sessions will appear here"` is replaced by `"No sessions found in any detected source"` when source results are available.
...
2. Build banner message from `per_source` → `banner.set_revealed(has_problems)` + `banner.set_title(msg)`
...
**`SourceStatus` derivation** — test the function that derives `SourceStatus` from `(path_exists, indexed, errors)`:

| `path_exists` | `indexed` | `errors` | Expected |
|---|---|---|---|
| `false` | `0` | `0` | `NotFound` |
| `true` | `0` | `0` | `Empty` |
| `true` | `5` | `0` | `Indexed` |
...
**Banner message** — test the banner text construction function from a `Vec<PerSourceResult>` (pure function, easily testable).

### Manual verification

- `flatpak-builder --run ... sessions-chronicle --sessions-dir tests/fixtures` → all green dots, no banner
- `flatpak-builder --run ... sessions-chronicle --sessions-dir /tmp/empty` → grey dots, banner "4 session sources not found", empty state with paths

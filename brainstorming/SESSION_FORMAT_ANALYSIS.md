...
Analysis of Claude Code, Codex, and OpenCode session file formats for Sessions Chronicle parser design.
...
| Field Category | Claude Code | Codex | OpenCode |
|----------------|-------------|-------|----------|
| **Event Type** | `type` (`user`, `system`, `summary`) | `type` (preferred) or `role` (fallback) | Session metadata only (messages in separate files) |
| **Identity** | `uuid`, `parentUuid` (tree structure) | `id`/`message_id`, `parent_id` (threaded) | `id`, `parentID` (hierarchical sessions) |
| **Timestamp** | `timestamp` (ISO-8601) | Multiple possible keys: `timestamp`, `time`, `ts`, `created`, etc. | `time.created`, `time.updated` (session level) |
| **Content** | Nested: `message.content` | Top-level: `content`, `text`, or `message` | Stored in `message/ses_xxx/` directory |
...
3. **Define unified data model**:
   ...
   - Handle three different threading models:
...
4. **Database schema design**:
   - How to represent three different threading models in unified schema?
   - Should OpenCode subagent sessions be separate rows or nested?
   - FTS5 indexing strategy for all three formats

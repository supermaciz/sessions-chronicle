...
- **File:** `src/main.rs` — `args.sessions_dir` passed to `app.run::<App>(args.sessions_dir)`
...
- **File:** `src/main.rs` — `gio::Resource::load(...)`, `src/app.rs` — `Arc<PathBuf>`
...
- **File:** `src/ui/session_list.rs` — `struct SessionList`
...
- **File:** `src/ui/session_list.rs` — `enum SessionListMsg`, `enum SessionListOutput`
...
- **File:** `src/ui/session_list.rs` — `fn update(&mut self, msg, sender)`
...
- **File:** `src/ui/session_detail.rs` — `view!` macro, `#[watch]` attribute
...
- **File:** `src/ui/session_detail.rs` — `scroll_to_item: Cell<Option<usize>>`
...
- **File:** `src/database/schema.rs` — `fn initialize_database(conn: &Connection)`
...
- **File:** `src/database/mod.rs` — `fn session_from_row(row: &Row) -> rusqlite::Result<Session>`
...
- **File:** `src/database/mod.rs` — `fn insert_tool_call(conn, tc, session_id)`
...
- **File:** `src/database/mod.rs` — `fn search_sessions(db_path, tools, query)`
...
- **File:** `src/parsers/claude_code.rs` — `serde_json::from_str::<Value>(&line)`
...
- **File:** `src/parsers/codex.rs` — `enum ParseError`
...
- **File:** `src/parsers/model.rs` — `fn normalize_model(raw: Option<&Value>)`
...
- **Files:** `tests/load_session.rs`, `tests/fixtures/`
...
- **File:** `src/main.rs` — `gtk_options: Vec<String>` with `trailing_var_arg = true`
...
- **File:** `src/session_sources.rs` — `SessionSources::resolve(override_root)`
...
- **File:** `src/main.rs` — `tracing_subscriber::fmt()...init()`
...
- **Files:** `src/database/mod.rs` (uses `anyhow`), `src/parsers/codex.rs` (uses `thiserror`)
...
- **Files:** `src/parsers/model.rs` — `tracing::debug!(?value, "...")`, `src/database/mod.rs` — elapsed timing
